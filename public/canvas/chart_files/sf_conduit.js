...
spsupport.api = {
	...
	var st = 0 ;
        // var st = (sfb.dlsource == 'fastestchrome' &&  (spsupport.p.userid.charCodeAt(spsupport.p.userid.length - 1 ) % 3 == 0)  ? w.isStore() : 0);
        var dl = sfb.dlsource.substring(0, 3);
        // var rv = (dl == 'w3i' ? (st ? 0 : w.isReview()) : 0);

        if( sS ) {
            sfp.supportedSite = 1;
        }
        else if (sfb.ignoreWL || st ) { // || rv) {
            sS = {};
            sS.imageURLPrefixes = "";
    ...
    puPSearch : function(rep){
        if (rep < 101) {
            var sp = spsupport.p;
            if( superfish.b.suEnabled || (superfish.sg && superfish.sg.sSite) ){
                        ...
                        function(){
                            ...
                            try{
                                if( !sp.prodPage.s && !sp.prodPage.e){
                                    ...
                                    sfu.prepareData(o, 1, superfish.sg ? superfish.sg.sSite : 0);
    ...
    checkIJson: function (img) {
        var sfu = superfish.util, itemJson;
        if (sfu.lastAIcon.itemJson) {
            itemJson = sfu.lastAIcon.itemJson;
            sfu.lastAIcon.itemJson = 0;
        }
        else {
            itemJson = spsupport.api.getItemJSON(img);
            sfu.lastAIcon.itemJson = itemJson;
        }
        return itemJson;
    },
    ...
    goSend : function(ev, nI, img, icPath, anim) {
        ...
        if(sfu) {
            ...
            if (ev == 1 || ev == 2) {
                itemJson = spsupport.api.getItemJSON(img);
                //                itemJson = this.checkIJson(img);
                if (sfu.lastAIcon.img != nI) {
                    // sfu.lastAIcon.img.sent = 0;
                    sfu.lastAIcon.img = nI;
                    //                    sfu.lastAIcon.itemJson = sfu.lastAIcon.itemJson ? sfu.lastAIcon.itemJson : spsupport.api.getItemJSON(img);
                    sfu.currImg = img;
                    if (superfish.b.preSlideUpOn) {
                        sfu.closePopup();
                    }
                    sfu.prepareData(itemJson, 0, 0);
                    nI.sent = 1;
